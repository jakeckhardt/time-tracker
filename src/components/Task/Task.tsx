...
export default function Task({ task, index, activeTask, handleTimer }: { task: Task, index: number, activeTask: number | null, handleTimer: (index: number, id: string) => void }) {
    ...
    return (
        <div key={"task-" + task.id} className={[styles.taskContainer, isOpen ? styles.open : " "].join(" ")}>
            <div className={[styles.task, activeTask === index ? styles.activeTask : ""].join(" ")}>
            ...
            <div className={styles.timeEntries}>
                {task.times.length === 0 ? (
                    ...
                    task.times.map((time, index) => (
                        <div key={`time-${task.id}-${index}`} className={styles.timeEntry}>
